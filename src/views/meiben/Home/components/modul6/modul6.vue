  ...
  <div class="modul3">
    <div class="modul3_bck">
      ...
      <div class="modul3_bck_box">
        ...
        <div class="top">
          <div class="top_items" v-for="(item,key) in home6.data_arr_below" :key="key">
    ...
    <div class="modul3_box">
      <div class="modul3_left">
        <div class="left1 wow animate__jackInTheBox">美国院校TOP 60统计</div>
        <div class="left2  wow animate__bounceInLeft" data-wow-delay='0.1s'>
          <div class="left2_top">
            ...
            <div class="t2">
              <span>{{home6.case_data}}</span>
              <!-- <img src="./image/icon_lvjt.png" alt=""> -->
             <!-- {{home6.up_value}}% -->
            </div>
          </div>
          <div class="left2_bottom ">
            <vue-seamless-scroll :data="home6.case_data_detail" class="seamless-warp" :class-option="classOption">
              <div class="left2_bottom_items" v-for="(item,key) in home6.case_data_detail" :key="key" @click="goDetails(item)">
                <div>
                  <span>●</span>
                  {{item.name}}
                </div>
                <div>{{item.ApplyDegree}}</div>
        ...
        <div class="left3 wow animate__bounceInLeft" data-wow-delay='0.3s'>
          <div v-for="(item,key) in home6.data_arr_left" :key="key">
            <div class="left3_t1">美本院校{{item.title}}案例统计</div>
            <div class="left3_t2">
              <span>{{item.num}}</span>
              <img src="./image/icon_lvjt.png" alt="">
              {{item.percentage}}%
            </div>
            <div class="left3_t3">Compared to ({{home6.year.min}}-{{home6.year.max}})</div>
          </div>
        </div>
      </div>
      <div class="modul3_right">
        ...
        <div class="right3 wow animate__bounceInRight" data-wow-delay='0.6s'>TOP 60案例占比情况</div>
        <div class="right4 wow animate__bounceInRight" data-wow-delay='0.7s'>
          <span>{{home6.case_percentage}}%</span>
          <!-- <img src="./image/icon_lvjt.png" alt=""> -->
          <!-- {{home6.proportion.up_value}}% -->
        </div>
        <div class="right5 wow animate__bounceInRight" data-wow-delay='0.8s'>Compared to ({{home6.year.min}}-{{home6.year.max}})</div>
        ...
        <div class="right7 wow animate__bounceInRight"  data-wow-delay='1s' >
            <vue-seamless-scroll :data="home6.school_data" class="seamless-warp" :class-option="classOption">
              <div class="right7_items" v-for="(item,key) in home6.school_data" :key="key" @click="goList('sqyxzwmc',item.AppliSchool)">
...
<script>
import { getHome6 } from '@/api/meiben.js'
...
export default {
  ...
  methods: {
    charts () {
      ...
      this.home6.data_school.forEach((item, index) => {
        const n = Number(item.type)
        item.color = color[n]
      })
      chart.data = this.home6.data_school
      console.log('chart.data', this.home6.data_school)
      series.minRadius = 45
      series.maxRadius = 45
    ...
    getData () {
      getHome6().then(res => {
        console.log('home6', JSON.parse(JSON.stringify(res.data)))
