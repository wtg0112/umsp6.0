  ...
  <div class="case">
        ...
        <div class="filtrate_xuanxiang">
          <div class="xuanxiang">
            ...
            <el-radio-group v-model="xuewei">
              <el-radio-button
                v-for="(item, key) in alOption.meiyan.xueweiList"
                :key="key"
                :label="item.value"
    ...
    <div class="center">
      <div class="center_left">
        <div style="color:#383874;">共{{total}}条案例</div>
        <div></div>
        <!-- <div>Active Data</div>
        <el-select
          v-model="limit"
        ...
        >
          <el-option
            v-for="item in alOption.meiyan.limitList"
            :key="item.value"
      ...
      <div class="center_right">
        ...
        >
          <el-option
            v-for="item in alOption.common.lqjgList"
            :key="item.value"
            :label="'录取结果：' + item.label"
            :value="item.value"
          ></el-option>
        </el-select>
        <!-- 排行榜 -->
        ...
        >
          <el-option
            v-for="item in alOption.meiyan.jpPaiming"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          ></el-option>
...
export default {
  ...
  data () {
    return {
      ...
      ielts: '',
      gre: '', // GRE分数
      gmat: '' // gmat 现在美研页面基本上都搭建完成，今天把六七屏做完整理的也过了一遍对接的接口，修改了大部分问题，
    }
  },
  created () {
    // 申请年份
